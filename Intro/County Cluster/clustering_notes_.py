# -*- coding: utf-8 -*-
"""
Created on Wed Oct 28 13:51:16 2015
...
# to do
# update all functions before k-means so that i can do hierarchical merge based on 
# either totaltoothcount or typetoothcount

# in the Point class we have distanceTypeCount and distanceToothSum 
...
class Point(object):
    ...
    consists of
    name: a string
    attrs: a list or array of numbers representing the feature vector
    ~ has a method for calculating distance between it and another point"""
...
class Cluster(object):
    """a collection of points stored in an array
    has a centroid and a string whose formal parameter is pointType"""
    def __init__(self, points, pointType):
        self.points = points
        self.pointType = pointType
        self.centroid = self.computeCentroid()
    def singleLinkageDist(self, other):
        """Returns the smallest distance between two clusters"""
    ...
    def maxLinkageDist(self, other):
        """Returns the largest distance between two clusters"""
    ...
    def update(self, points):
        """resets a cluster's points and then recalculates its centroid
        Returns the distance between the old and new centroids.
        ???if the length of the point list passed in is 0, it returns 0???
        so we only recalculate the centroid if we pass in more than 0 points
        """
    ...
    def members(self):
        ...
        for p in self.points:
            yield p
    def isIn(self, name):
        """check's, by name, to see if the cluster contains a certain point"""
        for p in self.points:
            if p.getName() == name:
                return True
        return False
    def toStr(self):
        """prints all the points(their names and values) in self"""
        result = ''
        for p in self.points:
            result = result + p.toStr() + ', '
        return result[:-2]
    def __str__(self):
        """prints the names of all points in self"""
        names = []
        for p in self.points:
    ...
    def computeCentroid(self):
        ...
        for p in self.points:
            totVals += p.getAttrs()
        #VVV WTF? Centroid is of whatever type the rest of the points are, ex. mammal
        centroid = self.pointType('mean',
                                   totVals/float(len(self.points)),
                                   totVals/float(len(self.points)))
        return centroid
    
    def avg_tooth_count(self):
        teeth = pylab.array([0.0,0.0,0.0,0.0])  
        for i in range(len(self.points)):        
            animal = self.points[i]
            animals_teeth = animal.getOriginalAttrs()
            print animals_teeth
            type_count = []
            for j in range(4):
                type_x = animals_teeth[2*j] + animals_teeth[2*j + 1]
                type_count.append(type_x)
            type_count = pylab.array(type_count)
            teeth += type_count
        teeth /= len(self.points)
        return teeth

class ClusterSet(object):
    """a set of clusters represented by a list of clusters"""
    ...
    def getClusters(self):
        """returns the clusters in our set"""
        return self.members[:]
    def mergeClusters(self, c1, c2):
        """removes two clusters from the set and adds their union
        Returns the two clusters which were merged"""
        points = []
        for p in c1.members():
            points.append(p)
        for p in c2.members():
    ...
    def findClosest(self, metric):
        """metric is one of the three linkage functions
        Returns the closest clusters(defined by the linkage function,metric)"""
    ...
    def mergeN(self, metric, numClusters = 1, history = [],
               toPrint = False):
        """reduces the set of clusters to numClusters clusters
        returns a list of tuples, 
        the i'th tuple contains the two clusters which were merged at the i'th iteration"""
    ...
    def __str__(self):
        """what happens when you turn a cluster into a string"""
        result = ''
...
def readMammalData(fName):
    ...
    The first list contains strings representing mammal names
    The second list contains lists of floats representing features"""
...
def buildMammalPoints(fName, scaling):
    """Returns a list of Mammal points, given a list of names and a list of lists of feature values"""
...
def test0(metric,numClusters = 2, scaling = '1/max', printSteps = False,
          printHistory = True):
    points = buildMammalPoints('mammalTeeth.txt', scaling)
    cS = ClusterSet(Mammal)
    for p in points:
        cS.add(Cluster([p], Mammal))
    history = cS.mergeN(metric, numClusters, toPrint = printSteps)
    if printHistory:
        print ''
...
#test0(Cluster.averageLinkageDist)
    
    
    
def teeth():
    """does hierarchical clustering on the mammal data set
    then makes bar charts showing the average # of each kind of tooth for both of the final clusters"""
    # to do
    # goal: plot average number of each kind of tooth for each cluster
    # 4 tooth types * 2 clusters = 8 bars
    # x-axis: tooth type, y-axis: number, bar-color: cluster    
    herb_carn = test0(Cluster.averageLinkageDist)
    tooth_types = ['incisor','canine','pre-molar','molar']
    #get averages for each cluster
    X = pylab.arange(4)
    clust1 = herb_carn[0]
    clust2 = herb_carn[1]
    clust1_teeth_avg = clust1.avg_tooth_count()
    clust2_teeth_avg = clust2.avg_tooth_count()
    print type(clust1_teeth_avg)
    pylab.figure()    
    pylab.bar(X,clust1_teeth_avg,width=.25,color='r')
    pylab.bar(X+.25,clust2_teeth_avg,width=.25,color='g')
    pylab.xticks(X+.25,tooth_types)
    pylab.title('avg # of each type of tooth, per group')
    pylab.figure()
    X_2 = pylab.array([3])
    total_1 = sum(clust1_teeth_avg)
    total_2 = sum(clust2_teeth_avg)
    pylab.bar(X_2,total_1,width=.5,color='r')
    pylab.bar(X_2+1,total_2,width=.5,color='g')
    pylab.xticks(X_2,['total # of teeth'])
    pylab.title('avg # of teeth per group')
    pylab.figure(figsize=(15,8))
    row_labels = ['cluster one','cluster two']
    col_labels = tooth_types[:] + ['total']
    clust1 = list(clust1_teeth_avg) + [total_1]
    clust2 = list(clust2_teeth_avg) + [total_2]
    cell_text = [clust1,clust2]    
    X = pylab.table(cellText=cell_text,rowLabels=row_labels,colLabels=col_labels,loc='center')
    pylab.axis('off')    
    X.scale(1,3)    
    pylab.show()
...
#teeth()
#Are we really sorting by tooth type? It's just as likely that we're sorting by number of teeth
#The carnivores have more teeth, that's what's responsible for the sorting.
...
class County(Point):
    ...
    def __init__(self, name, originalAttrs, normalizedAttrs = None,
        ...
        # ^^ makes a string of 0's and 1's representing on off for values
    ...
    for line in dataFile:
        ...
        for f in dataLine[2:]:
            try:
                f = float(f)
                features.append(f)
                # why length of features - 1??? not length of features
