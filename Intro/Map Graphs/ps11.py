...
def load_map(mapFilename):
    ...
    for line in map_file:
        ...
        graph.addEdge(WeightedEdge(source,destination,values[2],values[3]))
    map_file.close()
    return graph
...
# outdoor_distances = []
# 
# for edge in mit_map.edges.keys():
#     for tri_tuple in mit_map.edges[edge]:
#         sources.append(edge.getName())
#         destinations.append(tri_tuple[0].getName())
#         total_distances.append(tri_tuple[1])
#         outdoor_distances.append(tri_tuple[2])
# pylab.figure()
# cell_text = [sources,destinations,total_distances,outdoor_distances]
# pylab.table(cellText=cell_text,rowLabels=['sources','destinations','total distances','outdoor distances'])
# pylab.show()
#==============================================================================
...
def bruteForceSearch(digraph, start, end, path = [], visited = []):    
    """
    ...
    # start like this: write a program that will print all possible paths from start to end
    ...
    if start == end:
        ...
        if start not in visited: 
            path.append(start)
            visited.append(start)
            start_node = find_start_node(digraph,start)
            for next_building_tuple in digraph.childrenOf(start_node):
                next_building = get_dest_node(next_building_tuple)
                bruteForceSearch(digraph,str(next_building),end,path,visited)
                        
    

def find_start_node(digraph,building):
    """
    Returns the node in a digraph corresponding to the 'building' string
    """
    for node in digraph.nodes:
        if node.getName() == building:
            return node
            
def get_dest_node(tri_tup):
    """
