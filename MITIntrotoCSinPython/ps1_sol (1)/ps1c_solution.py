...
while True:
    ...
    for month in range(1,13):
        ...
        if balance <= 0:
            break
        # What if we used very tiny numbers? What if the initial low and high
        # were such that high - low = .00000001?
        # we're relying on knowing that if the difference is this small
        # then the balance is certainly non-positive.
        # In other words, if the above for loop breaks because it ran out of
        # months then high - low > .005, for sure. So the danger of returning
        # too small a payment value is averted
...
# Does the balance have to be negative for the while loop to breaK?
