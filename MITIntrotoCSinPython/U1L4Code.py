...
##epsilon = 0.1
##low = 0.0
##high = x # instead of the if, can do max(x, 1)
##if x < 1:
##    low = x
##    high = 1
##ans = (high + low)/2.0
##while abs(ans**2 - x) >= epsilon: # originally there was also ans < x
...
##   x = x + 1
##   print 'x =', x
##   x = g() # sets f1.x to None since g() returns nothing
##   #assert False
##   return x
##
##x = 3
##z = f1(x) # This assignment runs the print statements in f1(x) 
##print z
##
##def isEven(i):
...
##          return None
##    elif not isEven(pwr) and val < 0 and val > -1:
##        low = -1
##        high = val
##        ans = (low + high) / 2.0
...
##    return ans
##
##print findRoot(3, -.125, .001)
##print findRoot(2, 49.00, .001)
##
##def testFindRoot():
