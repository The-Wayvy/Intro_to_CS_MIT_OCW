...
##
### ???The swapping part??? mutating objects is weird as fuck
##def selSort(L):
...
##        minVal= L[i]
##        j = i + 1
##        while j < len(L): #this while loop finds the mi
##            if minVal > L[j]:
##                minIndx = j
##                minVal= L[j]
##            j += 1
##        temp = L[i]       # only using a copy of L[1]
##        L[i] = L[minIndx] # only mutable = mutable allows for funky
##        L[minIndx] = temp # object manipulation indirect substitution
##        print 'Partially sorted list =', L
### complexity O(len(L-1)*len(L-2)) so ~ O(n**2) where n = Len(L)
### more precisely ~ O(n**2 / 2)
##
####L = [35, 4, 5, 29, 17, 58, 0]
####selSort(L)
####print 'Sorted list =', L
